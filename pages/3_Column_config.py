...
st.markdown('column(label= , help= , width= , disabled="True")')
st.data_editor(df,
             column_config={
                 'command':st.column_config.Column(
                     label='Streamlit Commands',
...
st.data_editor(df,
               column_config={
                   'is_widget':st.column_config.CheckboxColumn(
                       label='위젯인가?',
                       default=False,)
    #                ), 'command': st.column_config.TextColumn( label='Streamlit Commands', help='streamlit **widget** commands',
    # default='st.',
    # max_chars=20,
    # validate='^st\.[a=z_]+$'
               })
...
df3=pd.DataFrame(
    {'meeting_date':
     [datetime(2024,2,5,12,30),
      ...
      datetime(2024,2,6,1,30),
      datetime(2024,2,2,11,30),]},
)
...
st.dataframe(df6,
             column_config={'score': st.column_config.ListColumn(
                 width='medium'
